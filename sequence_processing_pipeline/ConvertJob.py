...
class ConvertJob(Job):
    ...
    def _get_sample_sheet_basics(self):
        # assume path to sample-sheet exists, and sheet is valid.
        # otherwise, we would not be here.
        sheet = load_sample_sheet('Goober')

        # parse bioinformatics section to get a definitive list of
        # values we can use to reliably generate a list of project_names
        # (w/out qiita_ids) and qiita_ids. This includes cases where one
        # project is named as a superset of another.
    ...
    def copy_sequences(self, sample_name, source_project, dest_project):
        ...
        project_names = list(self.basics.keys())
        ...
        samples = self.basics[source_project]['samples']
        ...
        if sample_name in samples:
            for src_fp in samples[sample_name]['matching_files']:
