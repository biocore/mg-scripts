...
from os.path import join, dirname, abspath, exists, isdir, isfile, basename
...
class HumanFilterJob(TorqueJob):
    def __init__(self, root_dir, project, sample_sheet_path, output_directory, forward, reverse):
        super().__init__()
        ...
        self.stdout_log_path = join(dirname(self.job_script_path), 'something_%s_%s.out.log' % forward, reverse)
        self.stderr_log_path = join(dirname(self.job_script_path), 'something_%s_%s.err.log' % forward, reverse)
    ...
    def copy_directory_contents(self, source_directory, destination_directory, ends_with=None, contains=None,
                                starts_with=None, delete_after=False):
        ...
        for file in self.trim_file_obj.list:
            ...
            if size1 <= 500 or size2 <= 500:
                ...
                self.empty_file_list_txt.append(
                    join(partial_path, 'trimmed_sequences', filename1_short + '.fastp.fastq.gz'))
                self.empty_file_list_txt.append(size1)
                self.empty_file_list_txt.append(
                    join(partial_path, 'trimmed_sequences', filename2_short + '.fastp.fastq.gz'))
    ...
    def main(self, rundir, filter_db, a_trim, h_filter, chemistry, qiita_proj, copy_file, adapter_a, adapter_A, nprocs,
             final_output):
    ...
    def create_directories(self, dir, file_dir, output_dir, base_seq_dir, project, final_output_dir, trim_file, current_directory):
